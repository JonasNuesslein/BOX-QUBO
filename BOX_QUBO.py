...
class BOX_QUBO:
    ...
    def test(self):
        Q, bias = self.get_Q()
        for i in range(500): #int(len(self.D)*self.ratio)
...
def optimize(oracle, init_X, init_Y, ratio, training_length):
    box_qubo = BOX_QUBO(np.copy(init_X), np.copy(init_Y), ratio)
    box_qubo.train(20, 400)
